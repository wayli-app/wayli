...
console.log(
	`   otpauth URL: otpauth://totp/${config.issuer}:${config.email}?secret=${secret}&issuer=${config.issuer}`
);
...
console.log(
	`   Valid token '${validToken}': ${TOTPService.validateToken(validToken).isValid ? '✅' : '❌'}`
);
console.log(
	`   Invalid token '${invalidToken}': ${!TOTPService.validateToken(invalidToken).isValid ? '✅' : '❌'}`
);
...
console.log(
	`   Valid recovery code '${validRecoveryCode}': ${TOTPService.validateRecoveryCode(validRecoveryCode).isValid ? '✅' : '❌'}`
);
console.log(
	`   Invalid recovery code '${invalidRecoveryCode}': ${!TOTPService.validateRecoveryCode(invalidRecoveryCode).isValid ? '✅' : '❌'}`
);
console.log(
	`   Empty recovery code: ${!TOTPService.validateRecoveryCode(emptyRecoveryCode).isValid ? '✅' : '❌'}`
);
...
console.log(
	`   Invalid config: ${!TOTPService.validateConfig(invalidConfig).isValid ? '✅' : '❌'}`
);
