<script lang="ts">
	...
	import { Sun, Moon, User, BookOpen, Globe, MapPin, Calendar, Compass } from 'lucide-svelte';
	...
	let redirectUser = $state<string | null>(null);
		...
		(async () => {
			...
			try {
				const { data: setting } = await fluxbase
					.from('app.settings')
					.select('value')
					.eq('key', 'wayli.community_enabled')
					.maybeSingle();
				isEnabled = (setting as any)?.value === true || (setting as any)?.value === 'true';
			} catch {}

			// Check redirect user (for single-user instances)
			try {
				const { data: redirectSetting } = await fluxbase
					.from('app.settings')
					.select('value')
					.eq('key', 'wayli.landing_redirect_username')
					.maybeSingle();
				redirectUser = (redirectSetting as any)?.value ?? null;
			} catch {}
			...
			// If community disabled, redirect to user blog or show sign-in
			if (!isEnabled) {
...
{:else if !communityEnabled}
	<!-- Single-user mode: minimal sign-in page -->
	<div class="bg-background flex min-h-screen flex-col items-center justify-center p-4">
		<div class="w-full max-w-sm space-y-6 text-center">
			<div class="flex items-center justify-center gap-2">
				<Compass class="text-primary h-8 w-8" />
				<span class="text-foreground text-2xl font-bold">Wayli</span>
			</div>
		...
		>
			...
			<div class="relative mx-auto max-w-6xl px-4 py-20 text-center">
				<div class="flex items-center justify-center gap-3">
					<Compass class="h-10 w-10 text-white/80" />
		...
		<div class="mx-auto max-w-6xl px-4 py-10">
			...
			{#if latestEntries.length === 0 && travelers.length === 0}
				<div class="flex flex-col items-center justify-center py-20 text-center">
					<Compass class="text-muted-foreground mb-4 h-12 w-12 opacity-30" />
