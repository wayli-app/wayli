...
export const GET: RequestHandler = async ({ url, locals }) => {
	try {
		...
		if (includeLocations) {
			...
			const { data: locations, error: locationsError } = await query.order('created_at', {
				ascending: false
			});
		...
		if (includePOIs) {
			...
			const { data: pois, error: poisError } = await query.order('created_at', {
				ascending: false
			});
		...
		if (includeTrackerData) {
			...
			let allTrackerQuery = supabaseAdmin
				.from('tracker_data')
				.select(
					'user_id, tracker_type, device_id, recorded_at, location, altitude, accuracy, speed, heading, battery_level, is_charging, activity_type, created_at, geocode'
				)
				.eq('user_id', user.id);
			...
			const { data: allTrackerData, error: allTrackerError } = await allTrackerQuery.order(
				'recorded_at',
				{ ascending: true }
			);
			...
			if (allTrackerError) {
				...
				console.log(
					'API: All tracker data points fetched for mode calculation:',
				...
				const finalTrackerData =
					allTrackerData?.map((track, index) => {
