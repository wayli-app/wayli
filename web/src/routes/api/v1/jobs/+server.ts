...
export const GET: RequestHandler = async ({ locals }) => {
	try {
		...
		console.log(
			'[Jobs API] Raw database response - jobs:',
			jobs?.map((j) => ({ id: j.id, type: j.type, status: j.status, created_by: j.created_by }))
		);
		...
		console.log(
			'[Jobs API] Jobs by type:',
			Object.keys(jobsByType).map((type) => ({
				type,
				count: jobsByType[type].length,
				jobs: jobsByType[type].map((j) => ({
					id: j.id,
					status: j.status,
...
export const POST: RequestHandler = async ({ request, locals }) => {
	try {
		...
		console.log('[Jobs API] Job creation request:', {
			type,
			priority,
		...
		console.log(
			`[Jobs API] ✅ New job created: ${job.id} (${type}) with priority ${priority || 'normal'}`
		);
