      ...
      start(controller) {
        ...
        async function sendAllJobUpdates() {
          ...
          try {
            ...
            const allFinishedJobs = [...completedJobs.jobs, ...failedJobs.jobs, ...cancelledJobs.jobs];
            const recentFinishedJobs = allFinishedJobs.filter(job =>
              job.updated_at && new Date(job.updated_at) > new Date(tenSecondsAgo)
            );
