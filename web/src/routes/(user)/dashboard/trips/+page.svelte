<script lang="ts">
	...
	async function submitTrip(event?: Event) {
		...
		try {
			...
			closeTripModal();
			toast.success(wasEditing ? t('trips.tripUpdatedSuccessfully') : t('trips.tripCreatedSuccessfully'));
	...
	async function handleApproveTrips() {
		...
		try {
			...
			if (createdTripIds.length > 0) {
				...
				toast.success(t('trips.tripsApprovedWithImages', {
	...
	function formatDateRange(startDate: string | null | undefined, endDate: string | null | undefined): string {
		...
		const notSpecified = t('trips.notSpecified');
		if (start === notSpecified && end === notSpecified) {
			return t('trips.anyToAny');
		} else if (start === notSpecified) {
			return t('trips.anyTo', { date: end });
		} else if (end === notSpecified) {
			return t('trips.toPresent', { date: start });
		} else {
			return `${start} to ${end}`;
		}
	}
...
{t('trips.newTrip')}
			...
			>
				...
				<form on:submit|preventDefault={submitTrip} class="space-y-5">
					...
					<div>
						<label
							for="new-label"
							class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('trips.labels')}</label
						>
						<div class="space-y-3">
							<!-- Add new label -->
							<div class="flex gap-2">
							...
							/>
								...
								<div class="text-xs text-gray-500 dark:text-gray-400">
									{#if !uploadedImageUrl && !imageFile}
										...
										</button> {t('trips.basedOnTravelData')}
									{:else if suggestedImageUrl}
										<button
											...
											disabled={isSuggestingImage}
										>
											{isSuggestingImage ? t('trips.suggesting') : t('trips.suggestDifferentImage')}
										</button> {t('trips.basedOnTravelData')}
									{/if}
								</div>
							{/if}
	...
	{:else if filteredTrips.length === 0}
		<div class="py-12 text-center">
			...
			<h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-gray-100">{t('trips.noTripsFound')}</h3>
						...
						{:else}
							<div class="flex h-full w-full items-center justify-center">
								<div class="text-center">
									...
									<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{t('trips.noImageAvailable')}</p>
								</div>
							</div>
						{/if}
...
{t('trips.autoGeneratedLabel')}
					...
					<div class="flex min-h-0 flex-1 flex-col p-4">
						...
						>
							<span class="text-xs text-gray-500 dark:text-gray-400">
								{t('trips.updated')} {getRelativeTime(trip.updated_at)}
							</span>
							<button
		...
		<div class="flex flex-col h-full max-h-[70vh]">
			...
			{#if isLoadingSuggestedTrips && suggestedTrips.length === 0}
				<div class="flex items-center justify-center py-12">
					...
					<span class="ml-2 text-gray-600 dark:text-gray-400">{t('trips.loadingSuggestedTrips')}</span>
				</div>
								...
								<div class="flex items-start gap-4">
									...
									<div class="flex-1">
										...
										<div class="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
											<span>{t('trips.start')}: {format(new Date(trip.start_date), 'MMM d, yyyy')}</span>
											<span>{t('trips.end')}: {format(new Date(trip.end_date), 'MMM d, yyyy')}</span>
...
{t('trips.cancel')}
				...
				>
					...
					<span class="truncate">
						{t('trips.rejectTrips', { count: selectedSuggestedTrips.length, plural: selectedSuggestedTrips.length !== 1 ? 's' : '' })}
					</span>
				</button>
				<button
					on:click={() => {
				...
				>
					...
					<span class="truncate">
						{t('trips.approveTrips', { count: selectedSuggestedTrips.length, plural: selectedSuggestedTrips.length !== 1 ? 's' : '' })}
					</span>
				</button>
			</div>
		</div>
			...
			>
				...
				<div class="mb-6">
					<div class="flex items-center justify-between mb-2">
						<span class="text-sm font-medium text-gray-700 dark:text-gray-300">{t('trips.stepOf', { current: approvalProgress.currentStep, total: approvalProgress.totalSteps })}</span>
				...
				<div class="mb-6 space-y-3">
					<div class="flex items-center">
						...
						<div class="ml-3">
							<p class="text-sm font-medium text-gray-900 dark:text-gray-100">{t('trips.generateImages')}</p>
							<p class="text-xs text-gray-500 dark:text-gray-400">{t('trips.creatingImagesDescription')}</p>
						</div>
					</div>
					...
					<div class="flex items-center">
						...
						<div class="ml-3">
							<p class="text-sm font-medium text-gray-900 dark:text-gray-100">{t('trips.createTrips')}</p>
							<p class="text-xs text-gray-500 dark:text-gray-400">{t('trips.approvingTripsWithImages')}</p>
						</div>
					</div>
				</div>
