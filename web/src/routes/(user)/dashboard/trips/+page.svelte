	...
	function filterTrips() {
		...
		switch (selectedFilter) {
			case 'recent':
				...
				break;
			case 'long':
				filtered = filtered.filter((trip) => (trip.metadata?.distanceTraveled ?? 0) > 10000); // > 10km
				break;
			case 'short':
				filtered = filtered.filter((trip) => (trip.metadata?.distanceTraveled ?? 0) <= 10000); // ≤ 10km
				break;
			...
			for (let i = 0; i < selectedSuggestedTrips.length; i++) {
				...
				try {
					...
					if (singleImageResult?.results?.[0]?.success) {
						...
						const resultError =
							singleImageResult?.results?.[0]?.error || 'Failed to generate image';
...
<div class="space-y-6">
	...
	<div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
		...
		<div class="flex gap-2">
			...
			<button
				class="bg-primary hover:bg-primary/90 flex cursor-pointer items-center gap-2 rounded-lg px-4 py-2 font-medium text-white transition-colors"
				...
				<div class="p-8">
					...
					<form onsubmit={(e) => submitTrip(e)} class="space-y-5">
						...
						<div>
								...
								>
									<div class="mb-2 flex items-center justify-between">
										...
										{#if isSuggestingImage}
											<Loader2 class="text-primary dark:text-primary h-4 w-4 animate-spin" />
										{/if}
									</div>
							...
							<div class="relative mt-2 flex h-40 items-center justify-center rounded-lg bg-muted">
								...
								{#if suggestedImageUrl && !isSuggestingImage}
									<div
										class="bg-primary absolute top-2 right-2 rounded px-2 py-1 text-xs font-medium text-white"
									>
										Suggested
									</div>
								{/if}
								...
								{#if tripForm.start_date && tripForm.end_date && !isSuggestingImage}
									<div class="absolute top-2 left-2">
										...
										>
											{suggestedImageUrl || uploadedImageUrl
												? '🔄 Try different'
												: '✨ Suggest image'}
										</button>
									</div>
								...
								<div class="mt-2 text-xs text-muted-foreground">
									...
									{#if imageAttribution.photographerUrl}
										<a
											...
											class="text-primary hover:text-primary/80 dark:text-primary dark:hover:text-primary/80 underline"
										>
						...
						<div class="flex justify-end gap-3 pt-2">
							...
							<button
								type="submit"
								class="bg-primary hover:bg-primary/90 flex cursor-pointer items-center gap-2 rounded-lg px-5 py-2 font-medium text-white transition-colors"
								disabled={isSubmitting}
							>
								{#if isSubmitting}
									<Loader2 class="h-4 w-4 animate-spin" />
	...
	{:else}
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each filteredTrips as trip (trip.id)}
				<div
					class="relative flex h-full flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-border dark:bg-card"
				>
					<!-- Action Buttons -->
					...
					<!-- Trip Image (clickable → detail page) -->
					<a href="/dashboard/trips/{trip.id}" class="block">
						<div class="relative h-48 bg-gray-200 dark:bg-muted">
							{#if (trip.metadata?.distanceTraveled ?? 0) > 0}
								<div
								...
								>
									<Route class="text-primary dark:text-primary h-4 w-4" />
							...
							{#if trip.labels?.includes('auto-generated')}
								<div
									class="bg-primary absolute top-3 left-3 rounded px-2 py-1 text-xs font-medium text-white shadow"
								>
					...
					<div class="flex min-h-0 flex-1 flex-col p-4">
						<div class="mb-2 flex flex-wrap items-start justify-between gap-2">
							<h3 class="line-clamp-2 flex-1 text-lg font-semibold text-foreground">
								<a href="/dashboard/trips/{trip.id}" class="hover:text-primary">{trip.title}</a>
							</h3>
							<!-- Trip type badges -->
		...
		<div class="flex h-full max-h-[70vh] flex-col">
			...
			<div class="mt-4 flex gap-3 border-t pt-4 border-border">
				...
				<button
					...
					disabled={selectedSuggestedTrips.length === 0}
					class="bg-primary hover:bg-primary/90 focus:ring-primary dark:bg-primary dark:hover:bg-primary/90 flex flex-1 items-center justify-center rounded-lg px-4 py-2 font-medium text-white transition-colors focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
				>
	...
	{#if isApprovalInProgress}
		...
		>
			<div class="relative mx-4 w-full max-w-md rounded-2xl p-8 shadow-2xl bg-card" role="document">
				...
				<div class="bg-primary/5 dark:bg-primary/20 mb-6 rounded-lg p-4">
					<div class="flex items-center">
						{#if approvalProgress.step === 'generating-images'}
							<Loader2 class="text-primary dark:text-primary mr-3 h-5 w-5 animate-spin" />
				...
				{#if approvalProgress.step === 'complete'}
					<div class="flex justify-end">
						<button
							class="bg-primary hover:bg-primary/90 cursor-pointer rounded-lg px-5 py-2 font-medium text-white transition-colors"
