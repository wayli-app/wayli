		...
		<div class="rounded-xl border border-[rgb(218,218,221)] dark:border-[#23232a] bg-white dark:bg-[#23232a] p-6">
			<div class="mb-6">
				<div class="flex items-center gap-2">
					...
					<h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">OwnTracks Integration</h2>
				</div>
				<p class="mt-1 text-sm text-gray-600 dark:text-gray-300">
					Set up Wayli with OwnTracks by using the following API endpoint and key in your OwnTracks app.
				</p>
			</div>
			...
			<div class="space-y-4">
				<!-- API Endpoint -->
				<div>
					<label class="mb-1.5 block text-sm font-medium text-gray-900 dark:text-gray-100" for="owntracksEndpoint">API Endpoint</label>
					<div class="flex gap-2">
						<input
							...
							class="flex-1 rounded-md border border-[rgb(218,218,221)] dark:border-[#3f3f46] bg-gray-50 dark:bg-[#1a1a1a] px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:border-[rgb(37,140,244)] focus:outline-none focus:ring-1 focus:ring-[rgb(37,140,244)]"
						/>
						{#if data.owntracksEndpoint}
							<button
								type="button"
								on:click={() => data.owntracksEndpoint && copyToClipboard(data.owntracksEndpoint, 'API Endpoint')}
								class="flex items-center gap-2 rounded-md border border-[rgb(218,218,221)] dark:border-[#3f3f46] px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition-colors"
							>
				...
				<!-- API Key -->
				<div>
					<label class="mb-1.5 block text-sm font-medium text-gray-900 dark:text-gray-100" for="owntracksApiKey">API Key</label>
					<div class="flex gap-2">
						<input
							type="text"
						...
						{#if data.owntracksApiKey}
							<button
								type="button"
								on:click={() => data.owntracksApiKey && copyToClipboard(data.owntracksApiKey, 'API Key')}
								class="flex items-center gap-2 rounded-md border border-[rgb(218,218,221)] dark:border-[#3f3f46] px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition-colors"
							>
				...
				<form method="POST" action="?/generateApiKey" use:enhance>
					<button
						...
						class="flex items-center gap-2 rounded-md bg-[rgb(37,140,244)] px-4 py-2 text-sm font-medium text-white hover:bg-[rgb(37,140,244)]/90 transition-colors cursor-pointer"
					>
				...
				<div class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
					<h3 class="text-sm font-medium text-blue-800 dark:text-blue-200 mb-2">Setup Instructions:</h3>
