...
export function validateNodeEnvironmentConfig(): NodeEnvironmentConfig {
  ...
  const supabaseAnonKey = process.env.PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0';
  ...
  if (errors.length > 0) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('⚠️  Running in development mode with default values. Some features may not work correctly.');
    } else {
