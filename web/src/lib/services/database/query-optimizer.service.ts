...
class QueryOptimizerService {
  ...
  async getJobsWithStatus(userId: string, status?: string) {
    ...
    return this.cachedQuery(cacheKey, async () => {
      ...
      if (error) throw error;
      return data;
    }, 30 * 1000); // 30 seconds cache for jobs
  }
...
export const optimizedQueries = {
  ...
  // Get jobs
  getJobs: (userId: string, status?: string) =>
    queryOptimizer.getJobsWithStatus(userId, status),
