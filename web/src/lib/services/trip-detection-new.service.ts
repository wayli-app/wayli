// web/src/lib/services/trip-detection-new.service.ts
...
export class TripDetectionService {
	...
	private async processDateRangeInChunks(
		...
		while (hasMoreData) {
			...
			if (progressCallback) {
				...
				progressCallback(chunkProgress, `Processed ${currentOffset} points, found ${trips.length} trips`);
			}
				...
				if (updatedTripState.awayPoints.length >= config.minStatusConfirmationPoints) {
					...
					if (awayDuration >= config.minAwayDurationHours && !updatedTripState.currentTripStart) {
						...
						if (updatedTripState.lastHomePoint) {
							updatedTripState.currentTripStart = updatedTripState.lastHomePoint.timestamp;
	...
	private async finalizeTrip(
		...
		const totalDataPoints = tripState.currentTripLocations.reduce((sum, loc) => sum + loc.dataPoints, 0);
		...
		const trip: DetectedTrip = {
			...
			title: `Trip to ${tripState.currentTripLocations[0]?.cityName || 'Unknown'}`,
			...
			metadata: {
				...
				visitedCities: tripState.currentTripLocations.map(loc => loc.cityName),
				visitedCountries: tripState.currentTripLocations.map(loc => loc.countryName),
				visitedCountryCodes: tripState.currentTripLocations.map(loc => loc.countryCode),
				visitedLocations: tripState.currentTripLocations,
				isMultiCountryTrip: false, // Will be calculated later
				isMultiCityTrip: tripState.currentTripLocations.length > 1,
				...
				homeCity: 'Unknown', // Will be set later
				homeCountry: 'Unknown' // Will be set later
			},
			created_at: new Date().toISOString(),
			updated_at: new Date().toISOString()
		};

		console.log(`✅ Finalized trip: ${trip.start_date} to ${trip.end_date} (${duration}h)`);
		return trip;
	}
	...
	private async isUserHome(
		...
		for (const exclusion of tripExclusions) {
			if (exclusion.exclusion_type === 'city' && point.city_name === exclusion.value) {
				return true;
			}
		}

		// Check radius-based home detection
		if (homeAddress && homeAddress.lat && homeAddress.lon) {
	...
	private async processLocation(
		...
		// Basic validation
		if (!point.city_name || !point.country_name) {
			return null;
		}
