...
export class LocationCacheService {
   ...
   * Store data in cache
   */
  static setCachedData(filters: CacheKey, data: LocationData[], total: number, hasMore: boolean): void {
    try {
      const key = this.generateCacheKey(filters);
  ...
  static clearCache(): void {
    try {
      ...
      const locationKeys = keys.filter(key =>
        key.startsWith(this.CACHE_PREFIX) || key.startsWith(this.DAY_CACHE_PREFIX)
      );
  ...
  private static cleanupCache(): void {
    try {
      ...
      const entries = locationKeys.map(key => {
        try {
          const cached = localStorage.getItem(key);
          if (cached) {
