...
export class AuditLoggerService {
   ...
   */
  async logLogin(userId: string, success: boolean, request?: Request, metadata?: Record<string, unknown>): Promise<void> {
    ...
    await this.logEvent(eventType, description, severity, {
      ...metadata,
      success,
      user_id: userId
    }, request, userId);
  ...
  async logLogout(userId: string, request?: Request, metadata?: Record<string, unknown>): Promise<void> {
    await this.logEvent(
  ...
  async logRegistration(userId: string, request?: Request, metadata?: Record<string, unknown>): Promise<void> {
    await this.logEvent(
  ...
  async logJobEvent(
    jobId: string,
    eventType: AuditEventType.JOB_CREATED | AuditEventType.JOB_STARTED | AuditEventType.JOB_COMPLETED | AuditEventType.JOB_FAILED | AuditEventType.JOB_CANCELLED,
    userId: string,
    metadata?: Record<string, unknown>
  ): Promise<void> {
    const descriptions = {
    ...
    const severity = eventType === AuditEventType.JOB_FAILED ? AuditSeverity.MEDIUM : AuditSeverity.LOW;
