...
export class EnhancedPoiDetectionService {
	...
	async detectVisitsForUser(
		...
		try {
			...
			const updatedPois = await this.createOrUpdatePOIsWithStats(
				pois,
				userId,
				trackerData,
				finalConfig
			...
			if (existingPoi) {
				...
				const updatedRawData = {
					...
					total_visit_duration_minutes:
						((existingRawData.total_visit_duration_minutes as number) || 0) +
						visitStats.total_duration_minutes,
	...
	private averageLocation(
		loc1: { type: string; coordinates: number[] },
		loc2: { type: string; coordinates: number[] }
