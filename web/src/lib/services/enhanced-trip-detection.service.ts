...
export class EnhancedTripDetectionService {
  ...
  async detectSleepLocations(
    ...
    for (const [date, dayData] of Object.entries(dailyGroups)) {
      processedDays++;
      console.log(`📅 Processing day ${processedDays}/${Object.keys(dailyGroups).length}: ${date} (${dayData.length} data points)`);
      ...
      if (jobId) {
        ...
        const baseProgress = currentRange && totalRanges ? 20 + (currentRange * 60 / totalRanges) : 20;
      ...
      if (dayData.length < finalConfig.minDataPointsPerDay) {
        console.log(`⚠️ Skipping ${date}: insufficient data points (${dayData.length} < ${finalConfig.minDataPointsPerDay})`);
        skippedDays++;
        continue;
      }
      ...
      if (cityName && exclusions.some(exclusion => exclusion.exclusion_type === 'city' && cityName.toLowerCase().includes(exclusion.value.toLowerCase()))) {
        isAtHome = true;
        console.log(`🚫 Exclusion list: City '${cityName}' is in the exclusion list. Treating as 'home' and skipping further checks.`);
      } else {
        ...
        if (homeAddress?.address?.city && cityName) {
          ...
          isSameCityAsHome = cityName.toLowerCase().includes(homeCity.toLowerCase()) ||
                            homeCity.toLowerCase().includes(cityName.toLowerCase());
          hasHomeCity = true;
          console.log(`🏠 City comparison for ${date}: detected="${cityName}", home="${homeCity}" (from address.city), isSameCity: ${isSameCityAsHome}`);
          ...
          isSameCityAsHome = cityName.toLowerCase().includes(homeTown.toLowerCase()) ||
                            homeTown.toLowerCase().includes(cityName.toLowerCase());
          hasHomeCity = true;
          console.log(`🏠 Town comparison for ${date}: detected="${cityName}", home="${homeTown}" (from address.town), isSameCity: ${isSameCityAsHome}`);
          ...
          isSameCityAsHome = cityName.toLowerCase().includes(homeVillage.toLowerCase()) ||
                            homeVillage.toLowerCase().includes(homeVillage.toLowerCase());
          hasHomeCity = true;
          console.log(`🏠 Village comparison for ${date}: detected="${cityName}", home="${homeVillage}" (from address.village), isSameCity: ${isSameCityAsHome}`);
          ...
          isSameCityAsHome = cityName.toLowerCase().includes(homeMunicipality.toLowerCase()) ||
                            homeMunicipality.toLowerCase().includes(cityName.toLowerCase());
          hasHomeCity = true;
          console.log(`🏠 Municipality comparison for ${date}: detected="${cityName}", home="${homeMunicipality}" (from address.municipality), isSameCity: ${isSameCityAsHome}`);
        } else {
          console.log(`🏠 No city/town/village/municipality found in home address for comparison with "${cityName}"`);
        }

        // If both have city info, skip distance calculation entirely
        if (hasHomeCity && hasLocationCity) {
          isAtHome = isSameCityAsHome;
          console.log(`🏠 Fast path: Both have city info. isAtHome=${isAtHome} (city match: ${isSameCityAsHome}) - skipping distance calculation`);
          ...
          console.log(`🏠 Distance calculation for ${date}: ${distanceTime}ms, distance: ${distanceFromHome.toFixed(2)}km, isAtHome: ${isAtHome}`);
        } else {
          console.log(`🏠 No home address or coordinates for ${date}, skipping distance calculation`);
        }
      }
      ...
      if (!isExcluded && !isAtHome) {
        ...
        const reason = isAtHome ? `at home (distance: ${distanceFromHome.toFixed(2)}km${isSameCityAsHome ? ', same city' : ''})` : 'excluded';
  ...
  async generateSuggestedTrips(
    ...
    const sortedSleepLocations = sleepLocations.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  private detectTripPatterns(
    ...
    for (let i = 0; i < sortedSleepLocations.length; i++) {
      ...
      if (homeAddress?.address?.city && sleepLocation.cityName) {
        const homeCity = homeAddress.address.city;
        const isSameCityAsHome = sleepLocation.cityName.toLowerCase().includes(homeCity.toLowerCase()) ||
                                homeCity.toLowerCase().includes(sleepLocation.cityName.toLowerCase());
        if (isSameCityAsHome && !isAtHome) {
          isAtHome = true;
          console.log(`🏠 Trip pattern: Overriding isAtHome to true for ${sleepLocation.date}: same city as home (${sleepLocation.cityName})`);
        }
      } else if (homeAddress?.address?.town && sleepLocation.cityName) {
        // Fallback to town if city is not available
        const homeTown = homeAddress.address.town;
        const isSameCityAsHome = sleepLocation.cityName.toLowerCase().includes(homeTown.toLowerCase()) ||
                                homeTown.toLowerCase().includes(sleepLocation.cityName.toLowerCase());
        if (isSameCityAsHome && !isAtHome) {
          isAtHome = true;
          console.log(`🏠 Trip pattern: Overriding isAtHome to true for ${sleepLocation.date}: same town as home (${sleepLocation.cityName})`);
        }
      } else if (homeAddress?.address?.village && sleepLocation.cityName) {
        // Fallback to village if city and town are not available
        const homeVillage = homeAddress.address.village;
        const isSameCityAsHome = sleepLocation.cityName.toLowerCase().includes(homeVillage.toLowerCase()) ||
                                homeVillage.toLowerCase().includes(sleepLocation.cityName.toLowerCase());
        if (isSameCityAsHome && !isAtHome) {
          isAtHome = true;
          console.log(`🏠 Trip pattern: Overriding isAtHome to true for ${sleepLocation.date}: same village as home (${sleepLocation.cityName})`);
        }
      } else if (homeAddress?.address?.municipality && sleepLocation.cityName) {
        // Fallback to municipality if other fields are not available
        const homeMunicipality = homeAddress.address.municipality;
        const isSameCityAsHome = sleepLocation.cityName.toLowerCase().includes(homeMunicipality.toLowerCase()) ||
                                homeMunicipality.toLowerCase().includes(sleepLocation.cityName.toLowerCase());
        if (isSameCityAsHome && !isAtHome) {
          isAtHome = true;
          console.log(`🏠 Trip pattern: Overriding isAtHome to true for ${sleepLocation.date}: same municipality as home (${sleepLocation.cityName})`);
        }
      }
  ...
  private createTripFromSleepPattern(
    ...
    // Sort stays by date
    const sortedStays = sleepStays.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    const avgConfidence = sleepStays.reduce((sum, stay) => sum + stay.confidence, 0) / sleepStays.length;
    ...
    const title = uniqueCities.length === 1
