...
import type { TripGenerationData, TripExclusion, TrackerDataPoint, DetectedTrip, HomeAddress, SuggestedTrip } from '$lib/types/trip-generation.types';
...
export class JobProcessorService {
  ...
  private static async updateGeocodeWithError(point: {
    user_id: string;
    location: string | { type: string; coordinates: number[]; crs?: { type: string; properties: { name: string } } };
    geocode: unknown;
    recorded_at: string;
  ...
  private static async processTripGeneration(job: Job): Promise<void> {
    ...
    try {
      ...
      if (!startDate || !endDate) {
        console.log(`🔍 No start/end dates provided, finding available date ranges automatically...`);
        ...
        const dateRangeMessage = startDate || endDate
        ...
        dateRanges = await this.findAvailableDateRanges(userId, startDate, endDate);
        console.log(`📊 findAvailableDateRanges returned ${dateRanges.length} date ranges within provided range`);

        if (dateRanges.length === 0) {
          console.log(`❌ No available date ranges found within the provided range, completing job with error`);
          await JobQueueService.updateJobProgress(job.id, 100, {
            message: 'No available date ranges found within the provided date range. All dates are already covered by existing trips.',
            tripsGenerated: 0,
      ...
      if (useCustomHomeAddress && customHomeAddress) {
        ...
        try {
          ...
          if (geocodeResult) {
            ...
            console.log(`✅ Successfully geocoded custom home address: ${geocodeResult.display_name} (${geocodeResult.lat}, ${geocodeResult.lon})`);
          } else {
      ...
      for (let i = 0; i < dateRanges.length; i++) {
        ...
        console.log(`🔄 Starting to process date range ${i + 1}/${dateRanges.length}: ${range.startDate} to ${range.endDate}`);
        ...
        await JobQueueService.updateJobProgress(job.id, Math.min(100, Math.round(15 + (i * 60 / dateRanges.length))), {
        ...
        console.log(`📊 Starting to fetch GPS data for range ${range.startDate} to ${range.endDate} with sleep hours filtering`);
        const fetchStartTime = Date.now();

        // Use the optimized fetch method that filters data during the database query
        const { data: trackerData, totalFetched, totalFiltered, pageCount } = await this.fetchTrackerDataForRange(
          userId,
          range,
          {
        ...
        console.log(`📊 Sleep hours data fetch completed: ${totalFetched} total points → ${totalFiltered} sleep hours points in ${fetchTime}ms (${pageCount} pages)`);

        totalDataPoints += trackerData?.length || 0;

        if (!trackerData || trackerData.length === 0) {
          console.log(`⚠️ No sleep hours data found for range ${range.startDate} to ${range.endDate}`);
          continue;
        }

        await JobQueueService.updateJobProgress(job.id, Math.min(100, Math.round(20 + (i * 60 / dateRanges.length))), {
        ...
        console.log(`😴 Starting sleep location detection for ${trackerData.length} sleep hours data points`);
        const sleepDetectionStartTime = Date.now();

        const sleepLocations = await enhancedTripService.detectSleepLocations(trackerData, homeAddress, exclusions, config, job.id, i + 1, dateRanges.length);

        const sleepDetectionTime = Date.now() - sleepDetectionStartTime;
        console.log(`😴 Sleep location detection completed: ${sleepLocations.length} locations found in ${sleepDetectionTime}ms`);

        await JobQueueService.updateJobProgress(job.id, Math.min(100, Math.round(40 + (i * 60 / dateRanges.length))), {
        ...
        console.log(`🎯 Trip generation completed: ${suggestedTrips.length} trips generated in ${tripGenerationTime}ms`);

        allSuggestedTrips.push(...suggestedTrips);

        const rangeTime = Date.now() - rangeStartTime;
        console.log(`✅ Completed processing range ${i + 1}/${dateRanges.length}: ${range.startDate} to ${range.endDate} in ${rangeTime}ms`);

        await JobQueueService.updateJobProgress(job.id, Math.min(100, Math.round(60 + (i * 60 / dateRanges.length))), {
      ...
      console.log(`💾 Database save completed: ${savedTripIds.length} trips saved in ${saveTime}ms`);
      ...
      console.log(`✅ Sleep-based trip generation completed: ${savedTripIds.length} trip patterns created in ${totalTime}ms`);
      ...
      if (geojson.type === 'FeatureCollection' && geojson.features) {
        ...
        for (let i = 0; i < geojson.features.length; i++) {
          ...
          if (i % 1000 === 0 || currentTime - lastLogTime > 30000) {
            ...
            const eta = i > 0 ? ((geojson.features.length - i) / (i / elapsedSeconds)).toFixed(0) : '0';

            console.log(`📈 Progress: ${i.toLocaleString()}/${totalFeatures.toLocaleString()} (${progress}%) - Rate: ${rate} features/sec - ETA: ${eta}s - Imported: ${importedCount.toLocaleString()} - Skipped: ${skippedCount.toLocaleString()} - Errors: ${errorCount.toLocaleString()}`);
    ...
    try {
      ...
      for (let i = 0; i < waypoints.length; i++) {
        ...
        if (i % 10 === 0 || i === waypoints.length - 1) {
          ...
          console.log(`📈 Waypoints progress: ${i.toLocaleString()}/${waypoints.length.toLocaleString()} - Imported: ${importedCount.toLocaleString()} - Skipped: ${skippedCount.toLocaleString()} - Errors: ${errorCount.toLocaleString()}`);
      ...
      for (let i = 0; i < tracks.length; i++) {
        ...
        console.log(`📈 Tracks progress: ${(i + 1).toLocaleString()}/${tracks.length.toLocaleString()} - Total imported: ${importedCount.toLocaleString()} - Skipped: ${skippedCount.toLocaleString()} - Errors: ${errorCount.toLocaleString()}`);
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (i % 1000 === 0 || currentTime - lastLogTime > 30000) {
          ...
          console.log(`📈 Progress: ${i.toLocaleString()}/${totalLines.toLocaleString()} (${progress}%) - Rate: ${rate} lines/sec - ETA: ${eta}s - Imported: ${importedCount.toLocaleString()} - Skipped: ${skippedCount.toLocaleString()} - Errors: ${errorCount.toLocaleString()}`);
  ...
  private static async findAvailableDateRanges(
    ...
    try {
      ...
      console.log(`✅ Found ${filteredRanges.length} available date ranges for sleep pattern analysis:`);
      filteredRanges.forEach((range, index) => {
        const start = new Date(range.startDate);
        const end = new Date(range.endDate);
        const daysDiff = Math.ceil((end.getTime() - start.getTime()) / (24 * 60 * 60 * 1000)) + 1;
        console.log(`  Range ${index + 1}: ${range.startDate} to ${range.endDate} (${daysDiff} days)`);
      });
  ...
  private static async fetchTrackerDataForRange(
  ...
  ): Promise<{ data: TrackerDataPoint[]; totalFetched: number; totalFiltered: number; pageCount: number }> {
    console.log(`📊 Fetching tracker data for range ${range.startDate} to ${range.endDate} with sleep hours filtering`);
    ...
    for (let chunkStart = new Date(startDate); chunkStart <= endDate; chunkStart.setDate(chunkStart.getDate() + chunkSize)) {
      ...
      while (chunkHasMore) {
        chunkPageCount++;
        console.log(`📄 Fetching page ${chunkPageCount} (offset: ${chunkOffset}) for chunk ${chunkStartStr} to ${chunkEndStr}`);
        ...
        if (!pageData || pageData.length === 0) {
          ...
          console.log(`📄 Page ${chunkPageCount}: Got ${pageData.length} data points, filtered to ${filteredPageData.length} sleep hours points`);
    ...
    console.log(`📊 Sleep hours data fetch completed: ${totalFetched} total points → ${totalFiltered} sleep hours points (${pageCount} pages across all chunks)`);
