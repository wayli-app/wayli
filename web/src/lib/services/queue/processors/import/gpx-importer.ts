...
export async function importGPXWithProgress(
  ...
  try {
    ...
    for (let i = 0; i < waypoints.length; i++) {
      ...
      if (!isNaN(lat) && !isNaN(lon)) {
        ...
        if (!error) {
          ...
          if (err.code === '23505') skippedCount++; else { errorCount++; console.error(`❌ Error inserting waypoint ${i}:`, error); }
        }
      } else {
        skippedCount++;
      }
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      const trackPoints = trackSegments.flatMap((seg: any) => ensureArray(seg?.trkpt ?? []));
      ...
      for (let j = 0; j < trackPoints.length; j++) {
        ...
        if (!isNaN(lat) && !isNaN(lon)) {
          ...
          if (!error) importedCount++; else {
            const err = error as { code?: string };
            if (err.code === '23505') skippedCount++; else { errorCount++; console.error(`❌ Error inserting track point ${j} in track ${i}:`, error); }
          }
        } else {
          skippedCount++;
        }
...
function safeGetCountryForPoint(lat: number, lon: number): string | null {
  try { return getCountryForPointExternal(lat, lon); } catch (e) { console.warn('Failed to get country for point:', e); return null; }
}

function safeNormalizeCountryCode(countryCode: string | null): string | null {
  try { return normalizeCountryCodeExternal(countryCode); } catch (e) { console.warn('Failed to normalize country code:', e); return null; }
}
