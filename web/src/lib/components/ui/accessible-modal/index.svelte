<script lang="ts">
  ...
  import { FocusManager, ariaHelpers, keyboardNavigation, screenReader } from '$lib/accessibility/accessibility-utils';
    ...
    >
      ...
      {#if title || showCloseButton}
        <div class="flex items-start justify-between mb-6">
          {#if title}
            <h2 id={titleId} class="text-2xl font-bold text-gray-900 dark:text-gray-100 pr-8">
              {title}
            </h2>
          {/if}
          {#if showCloseButton}
            <button
              class="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors cursor-pointer p-2 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2"
