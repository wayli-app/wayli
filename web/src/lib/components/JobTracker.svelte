...
import { toast } from 'svelte-sonner';
	...
	export function startMonitoring() {
		...
		sseService = new SSEService({
			...
			onJobUpdate: (jobs) => {
				...
				// Check if any jobs passed the filter
				const filteredJobs = jobs.filter(job => !jobType || job.type === jobType);
				...
				for (const [jobId, job] of currentJobs.entries()) {
					if (job.status === 'completed' || job.status === 'failed' || job.status === 'cancelled') {
						...
						if (now - jobTime > 30000) { // 30 seconds
			...
			onJobCompleted: (jobs) => {
				...
				if (showToasts) {
					for (const job of jobs) {
						...
						if (job.status === 'completed' && !completedJobIds.has(job.id) && isRecentCompletion) {
						...
						} else if (job.status === 'failed' && !completedJobIds.has(job.id) && isRecentCompletion) {
	...
	let activeJobsCount = $derived(getActiveJobsMap().size);
</script>
