...
export const nonNegativeNumberSchema = z.number().nonnegative({ message: 'Must be a non-negative number' });
...
export const jobSchema = z.object({
  id: uuidSchema.optional(),
  type: z.enum(['reverse_geocoding_missing', 'data_import', 'trip_generation', 'poi_visit_detection']),
...
export const geocodeSearchSchema = z.object({
  q: z.string().min(3, 'Search query must be at least 3 characters').max(500, 'Search query too long')
});
...
export const fileUploadSchema = z.object({
  file: z.instanceof(File, { message: 'File is required' }),
  maxSize: z.number().positive().default(1024 * 1024 * 1024), // 1GB default
...
export const validateWithSchema = <T>(schema: z.ZodSchema<T>, data: unknown): { success: true; data: T } | { success: false; error: string } => {
  try {
...
export const validatePartial = <T>(schema: z.ZodSchema<T>, data: unknown): { success: true; data: Partial<T> } | { success: false; error: string } => {
  try {
