 ...
 * @deprecated Use withBodyValidation instead
 */
export function validateRequest<T>(schema: z.ZodSchema<T>): RequestHandler {
	return withBodyValidation(schema, async () => {
		// This is a placeholder - the actual handler should be provided
		return errorResponse('Handler not provided', 500);
	});
}

/**
 * Legacy function for backward compatibility
 * @deprecated Use withQueryValidation instead
 */
export function validateQueryParams<T>(schema: z.ZodSchema<T>): RequestHandler {
	return withQueryValidation(schema, async () => {
		// This is a placeholder - the actual handler should be provided
		return errorResponse('Handler not provided', 500);
	});
}
...
/**
 * Legacy function for backward compatibility
 * @deprecated Use withPathValidation instead
 */
export function validatePathParams<T>(schema: z.ZodSchema<T>): RequestHandler {
	return withPathValidation(schema, async () => {
		// This is a placeholder - the actual handler should be provided
		return errorResponse('Handler not provided', 500);
	});
}
...
export function createValidatedHandler<T>(
	schema: z.ZodSchema<T>,
	handler: (data: T, context: ValidatedRequestContext<T>) => Promise<Response>
): RequestHandler {
	return withBodyValidation(schema, async (validatedContext) => {
		return handler(validatedContext.validatedData, validatedContext);
	});
}
