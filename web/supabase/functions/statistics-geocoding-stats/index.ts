import { isRetryableError } from '../_shared/geocoding-utils.ts';
import {
	setupRequest,
	authenticateRequest,
	successResponse,
	...
	try {
		...
		if (req.method === 'GET') {
			...
			const cacheAge = cachedStats?.last_calculated
				? (Date.now() - new Date(cachedStats.last_calculated).getTime()) / (1000 * 60)
				: null; // Age in minutes
			...
			if (cachedStats && cacheAge !== null && cacheAge < 5 && !forceRefresh) {
				...
				return successResponse({
					...
					success_rate:
						cachedStats.total_points > 0
							? ((cachedStats.geocoded_points || 0) / cachedStats.total_points) * 100
							: 0,
			...
			logInfo(
				'Cache expired or missing, calculating fresh statistics',
				'STATISTICS-GEOCODING-STATS',
				{
					userId: user.id,
			...
			while (true) {
				...
				if (offset > maxSamples) {
					logInfo(
						`Reached max samples limit (${maxSamples}), stopping data fetch`,
						'STATISTICS-GEOCODING-STATS'
					);
					break;
				}
			...
			for (const point of sampleTrackerData) {
				...
				if (
					!geocode ||
					(typeof geocode === 'object' && geocode !== null && Object.keys(geocode).length === 0)
				) {
					nullOrEmptyGeocodes++;
				} else if (
					geocode &&
