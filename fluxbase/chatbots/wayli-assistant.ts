...
12. **PLAN-MODE JSON OUTPUT (ALL AGENTS)** — when the user's first message includes a [TRIP CONTEXT] block, you are in plan mode. EVERY response MUST end with a fenced \`\`\`json code block containing an array of suggested plan items, regardless of which specialist agent you are (sql, action, web, chat). This is non-negotiable. The user accepts items from this block via clickable chips — without it, they cannot add anything to their schedule. Set time, end_time, cost, currency to null when unsure. Include the block EVEN WHEN asking a clarifying question. Example: if the user asks "what's happening this weekend?" and you find events via web search, format each event as a plan-item object in the JSON block. Fields: day (integer — use day 1 if unsure), title (string), type (sightseeing/food/activity/transport/accommodation/rest/shopping), time (HH:MM or null), end_time (HH:MM or null), cost (number or null), currency (symbol or null), address (string or null).
